...
// Load loads standard skill directories containing SKILL.md files.
...
// Clone isolates slices so registered skills cannot be mutated by callers.
func Clone(skill Skill) Skill {
	skill.TriggerPhrases = append([]string(nil), skill.TriggerPhrases...)
	return skill
}

// Matches reports whether the skill should activate for the input.
func Matches(skill Skill, input string) bool {
	if skill.Match != nil && skill.Match(input) {
		return true
	}
