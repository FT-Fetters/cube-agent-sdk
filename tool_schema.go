...
func cloneToolParametersSchema(schema *ToolParametersSchema) *ToolParametersSchema {
	if schema == nil {
		return nil
	}
	cloned := *schema
	if len(schema.Properties) > 0 {
		...
		for name, property := range schema.Properties {
			propertyCopy := cloneToolParametersSchema(&property)
			if propertyCopy != nil {
	...
	cloned.Items = cloneToolParametersSchema(schema.Items)
	return &cloned
}
