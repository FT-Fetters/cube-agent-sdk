...
func (c *MCPStdioClient) Tools(ctx context.Context) ([]Tool, error) {
...
func cloneMCPToolDescriptors(tools []MCPToolDescriptor) []MCPToolDescriptor {
	...
	for i, tool := range tools {
		cloned[i] = tool
		cloned[i].InputSchema = cloneAnyMap(tool.InputSchema)
	}
	return cloned
}
...
func (c *MCPStdioClient) mcpError(operation string, toolName string, err error) error {
	...
	wrapped := agentError(ErrorCategoryMCP, operation, err)
...
func (t *mcpStdioTool) Call(ctx context.Context, call ToolCall) (ToolResult, error) {
	...
	if len(result.StructuredContent) > 0 {
		metadata["mcpStructuredContent"] = cloneAnyMap(result.StructuredContent)
	}
	return ToolResult{
		CallID:   call.ID,
		Name:     call.Name,
