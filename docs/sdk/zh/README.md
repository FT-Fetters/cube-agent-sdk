...
- [MCP](./mcp.md)：MCP server 元数据，以及 stdio、HTTP、SSE 工具桥接。
- [会话](./sessions.md)：reset、snapshot、restore 和 fork。
- [Eval 与 Replay](./evals.md)：scripted models、golden transcripts、replay 和
  测试断言。
- [审批](./approvals.md)：内置策略和生产审批流程。
- [可观测性](./observability.md)：hooks、observers、稳定遥测命名和脱敏遥测。
...
- [生产集成](./production.md)：真实部署时的集成和生产观测清单。
...
- OpenAI-compatible chat completions、OpenAI Responses 和 Anthropic Messages
  适配器，包含 provider 原生流式输出支持。
- 工具描述、工具参数 schema 和执行前校验。
- MCP stdio、HTTP、SSE clients 和 MCP 到工具的桥接。
- 会话 reset、snapshot、restore 和 fork。
- 用于 CI 测试的 scripted model、eval transcript、replay 和断言 helpers。
...
- MCP server 二进制文件或 URL、部署、进程监管和运行权限。
- 人类审批 UI 或业务策略集成。
- 为 snapshots、session records 和 event logs 提供持久化存储适配器、加密、访问控制、保留策略和迁移。
- 日志、指标和 tracing 导出器。
- 密钥管理、速率限制、发布策略和生产监控。
