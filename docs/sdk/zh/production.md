...
8. 为每个生产工具设置 `ToolSafety`，包含风险、timeout、最大并发、最大结果字节数，以及适用的副作用 scope。
9. 对模型失败率、provider 限流、工具失败、stream latency、本地工具限制拒绝和 telemetry 缺失配置告警。
10. 文档化隐私红线，并测试禁止字段不会进入 logs、metric labels、traces、span
   events 或 baggage。
11. 发布前运行本地 fake-provider 测试、OpenTelemetry 示例测试，以及可选 live-provider
    测试。
...
- `agent.tool.timeout_*`、`agent.tool.max_*`、`agent.tool.scope.*` 和
  `agent.tool.business_reason.hash` 用于解释本地工具 guardrails，同时不会暴露原始 scope value 或业务原因。
- `agent.run_id`、`agent.request_id`、`agent.parent_request_id` 和
  `agent.trace_id` 用于 logs 和 traces 关联。

## 工具安全

把工具执行当作生产安全边界处理。审批决定某次调用能否执行；`ToolSafety` 限制获批后最多能发生什么。MCP 工具、文件工具、网络工具，以及任何有副作用的自定义工具都应同时配置两者。

推荐基线：

1. 使用默认拒绝审批：组合 `AllowToolsApproval`、`AllowRisksApproval` 和应用自己的 `ApprovalFunc`。
2. 每个工具都声明风险。只读工具也应有 schema 和限制；write/destructive 工具还应绑定 scope 和业务原因。
3. 工具 timeout 应低于下游服务或进程 timeout。工具实现应遵守 `ctx.Done()` 并在取消后停止工作。
4. 设置 `MaxConcurrency`，避免本地 fan-out 耗尽文件、进程、socket、浏览器 session 或远端 API。
5. 设置 `MaxResultBytes`，避免工具输出冲击模型上下文或遥测管线。大体积产品数据应存入产品存储，只返回引用。
6. 将 write/destructive 工具绑定到窄 `ToolScope`，例如 tenant、repository、文件根目录、account、queue 或 service name。审批策略可以检查原始值；observation 只导出 scope 数量和 hash。
7. 用 `BusinessReason` 放组织要求的工单 ID 或策略原因。Observation 只导出 hash。
8. 对 `ErrToolConcurrencyLimitExceeded`、`ErrToolResultTooLarge`、`context.DeadlineExceeded` 和审批拒绝升高配置告警。

MCP 工具：发现工具后先用 `ToolWithSafety` 包装再注册。MCP server credentials 和 environment 不应进入遥测；优先使用只读 server 模式；把读写能力拆到不同 server instance；按 server 或 tool 分配 scope。不要把远端 MCP descriptor 的描述当作安全策略。

文件工具：限制在 allowlist root 或虚拟文件系统内，在工具实现中拒绝 path traversal 和 symlink escape，把 destructive 操作标成 `ToolRiskDestructive`，文件可能很大时返回有界摘要而不是完整内容。

网络工具：使用 host 或 service allowlist，限制 response size，设置 request timeout，除非明确需要否则不要转发凭据，并把完整 URL、headers、cookies 和 response bodies 视为敏感数据。尽可能返回 host/status/count metadata，而不是原始 payload。
...
- tool schema hashes、tool scope hashes、tool business-reason hashes、tool result metadata keys、skill names 和 approval reason text。
...
- 工具 timeout、result-size 或 concurrency-limit 拒绝高于预期基线。
...
- Tool health：validation、approval、execution durations、已配置 timeout、concurrency/result limits、scope counts、error category、tool risk 和受控 tool name 维度。
...
- 不要把原始工具结果内容、原始 scope value 或原始工具业务原因写入通用遥测。产品数据应进入带产品访问控制的产品存储。
- 把 session snapshots、session records 和 event logs 当作用户内容处理，配置存储加密、访问控制、保留策略和迁移评审。
- 不要把 provider credentials、runtime configuration、raw tool payloads 或 raw telemetry 写入 session event metadata。
